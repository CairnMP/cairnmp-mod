...
{
    private static readonly HarmonyLib.Harmony BivouacDiagnosticsHarmony = new("CairnMultiplayerMod.BivouacDiagnostics");
    private static bool _bivouacDiagnosticsPatchesInstalled;
    private static bool _bivouacDiagnosticsPatchesFailed;
    private static readonly System.Collections.Generic.HashSet<IntPtr> TapingInitAttempted = new();
    ...
    internal static void InstallBivouacDiagnosticsPatches()
    {
        ...
        try
        {
            ...
            Mod.LogDebug("[BivouacDebug] Diagnostic patches installed");
        }
        catch (Exception ex)
        {
            _bivouacDiagnosticsPatchesFailed = true;
            Mod.Log.Warning($"[BivouacDebug] Diagnostic patch install failed: {ex.Message}");
        }
    }
        ...
        string finalizerMethodName = null)
    {
        ...
        BivouacDiagnosticsHarmony.Patch(original, prefix: new HarmonyMethod(prefix),
    ...
    private static class BivouacDiagnosticPatchMethods
    {
            ...
            Exception __exception)
        {
            ...
            SafeLog("TapingActivateException", () =>
                $"{__exception.GetType().Name}:{FirstLine(__exception.Message)} readiness={DescribeTapingReadiness(__instance)}");
        ...
        private static void SafeLog(string eventName, Func<string> build)
        {
            ...
            try
            {
                Mod.LogDebug($"[BivouacDebug] event={eventName} {build()}");
            }
            catch (Exception ex)
            {
                Mod.Log.Warning($"[BivouacDebug] event={eventName} log failed: {ex.GetType().Name}:{FirstLine(ex.Message)}");
            }
        }
        ...
        private static void PrepareTapingSelection(TapingFingersManager manager)
        {
            ...
            try
            {
                ...
                {
                    Mod.Log.Warning("[BivouacDebug] SelectHand reflection repair unavailable");
                    return;
                }
            ...
            {
                Mod.Log.Warning($"[BivouacDebug] SelectHand repair failed: {ex.GetType().Name}:{FirstLine(ex.Message)}");
            }
        }
        ...
        private static bool EnsureCurrentModel(TapingFingersManager manager)
        {
            ...
            {
                SafeLog("TapingCurrentModelFailed", () => $"{ex.GetType().Name}:{FirstLine(ex.Message)}");
                return false;
            }
        }

        private static int ResolveCount(System.Type modelsType, object models)
        {
        ...
        private static object ResolveIndex(System.Type modelsType, object models, int index)
        {
        ...
        private static bool TryStartTapingFallback(TapingFingersManager manager)
        {
            ...
            {
                SafeLog("TapingStartBypassFailed", () => $"{ex.GetType().Name}:{FirstLine(ex.Message)}");
                return false;
            }
        }
        ...
        private static void CleanupTapingPartialState(TapingFingersManager manager, string reason)
        {
            ...
            {
                Mod.Log.Warning($"[BivouacDebug] Cleanup failed after {reason}: {ex.GetType().Name}:{FirstLine(ex.Message)}");
            }
        }
        ...
        private static object GetCurrentModelSafe(TapingFingersManager manager)
        {
            try
            {
        ...
        private static void TryInvokePrivate(TapingFingersManager manager, string methodName)
        {
            try
            {
                ...
                if (method == null)
                {
                    Mod.Log.Warning($"[BivouacDebug] {methodName} method not found");
                    return;
                }

                method.Invoke(manager, null);
                Mod.LogDebug($"[BivouacDebug] {methodName} invoked for TapingFingersManager");
            }
            catch (Exception ex)
            {
                Mod.Log.Warning($"[BivouacDebug] {methodName} invoke failed: {ex.GetType().Name}:{FirstLine(ex.Message)}");
            }
        }
        ...
        private static void InvokeExitCallback(Il2CppSystem.Action callback, string reason)
        {
            ...
            {
                callback.Invoke();
                Mod.LogDebug($"[BivouacDebug] Exit callback invoked after {reason}");
            }
            catch (Exception ex)
            {
                Mod.Log.Warning($"[BivouacDebug] Exit callback failed after {reason}: {ex.GetType().Name}:{FirstLine(ex.Message)}");
            }
        }
    }
}
