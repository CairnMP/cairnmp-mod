...
public static unsafe partial class CairnGameApi
{
    ...
    public static bool TryCaptureWeather(out WeatherSyncData state)
    {
        ...
        {
            Mod.Log.Warning($"[WeatherSync] Capture failed: {ex.Message}");
            return false;
        }
    }
    ...
    private static WeatherManager TryGetWeatherManager()
    {
        ...
        var behaviour = FindMonoBehaviourByName("WeatherManager", ref _weatherManagerBehaviourCached, ref _lastWeatherManagerSearchFrame);
    ...
    private static void LogRemoteWeatherApplyFailure(string reason)
    {
        ...
        Mod.Log.Warning($"[WeatherSync] Apply failed: {reason}");
    }
}
