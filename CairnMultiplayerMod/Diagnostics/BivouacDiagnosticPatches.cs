...
internal static unsafe partial class BivouacDiagnostics
{
    ...
    private static class BivouacDiagnosticPatchMethods
    {
        ...
        private static void SafeLog(string eventName, Func<string> build)
        {
            ...
            {
                Mod.Log.Warning($"[BivouacDebug] event={eventName} log failed: {ex.GetType().Name}:{GameInterop.FirstLine(ex.Message)}");
            }
        }
        ...
        private static void PrepareTapingSelection(TapingFingersManager manager)
        {
            ...
            {
                Mod.Log.Warning($"[BivouacDebug] SelectHand repair failed: {ex.GetType().Name}:{GameInterop.FirstLine(ex.Message)}");
            }
        }
        ...
        private static void CleanupTapingPartialState(TapingFingersManager manager, string reason)
        {
            ...
            {
                Mod.Log.Warning($"[BivouacDebug] Cleanup failed after {reason}: {ex.GetType().Name}:{GameInterop.FirstLine(ex.Message)}");
            }
        }
        ...
        private static void TryInvokePrivate(TapingFingersManager manager, string methodName)
        {
            ...
            {
                Mod.Log.Warning($"[BivouacDebug] {methodName} invoke failed: {ex.GetType().Name}:{GameInterop.FirstLine(ex.Message)}");
            }
        }
        ...
        private static void InvokeExitCallback(Il2CppSystem.Action callback, string reason)
        {
            ...
            {
                Mod.Log.Warning($"[BivouacDebug] Exit callback failed after {reason}: {ex.GetType().Name}:{GameInterop.FirstLine(ex.Message)}");
            }
        }
    }
}
