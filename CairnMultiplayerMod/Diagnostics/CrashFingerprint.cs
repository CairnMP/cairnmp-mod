...
/// <summary>
/// Builds the grouping key sent to the API alongside a crash report.
///
/// The server can only guess which parts of a report are stable; the mod knows.
/// A fingerprint made of the failing site (context, exception type, first frame
/// inside our own code) keeps every occurrence of one bug in a single console
/// group, no matter how the message or the deeper frames vary.
/// </summary>
public static class CrashFingerprint
{
    ...
    /// <summary>API cap on the fingerprint column.</summary>
    ...
    /// nothing stable to key on — the API then falls back to deriving a
    /// signature from the message and stack trace.
    /// </summary>
