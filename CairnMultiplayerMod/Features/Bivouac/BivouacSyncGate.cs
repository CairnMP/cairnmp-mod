...
namespace CairnMultiplayerMod.Features.Bivouac;
...
internal sealed class BivouacSyncGate
{
    ...
    internal BivouacSyncGate(NetworkManager network, IMultiplayerPanel panel)
    {
        _network = network;
        _panel = panel;
    }
    ...
    {
        Mod.Log.Msg("[State] Gameplay sync suspended for bivouac");
        _suspendedSince = Time.unscaledTime;
        _nextDebugLogAt = 0f;
        PausePatch();
        Mod.Instance.ResetSyncTimers();
        ...
        Mod.Instance.SetLocalState(PlayerState.Loading);
        // Remember the pawn's position on entry: used by the anti-lockup guard
        // (a pawn that has moved = we're climbing again).
        _hasSuspendPawnPos = LocalPlayerApi.TryGetPose(out _suspendPawnPos, out _);
        _stuckSince = 0f;
        LogPhase("enter");
    }
    ...
    private void ExitBivouac()
    {
        ...
        Mod.Log.Msg("[State] Gameplay sync resumed");
        Mod.Instance.ResetSceneBoundSyncState();
    }

    private bool ShouldSuspend()
    {
        if (SceneRoles.IsBivouac(Mod.Instance.CurrentScene))
        {
            _stuckSince = 0f;
            return true;
        }
    ...
    {
        if (!SceneRoles.IsGameplayRoot(Mod.Instance.CurrentScene))
            return false;
    ...
    private void PausePatch()
    {
        ...
        Mod.Log.Msg("[State] Netplay SetFrame patch paused for bivouac");
    }
    ...
    private void ResumePatch(bool immediate)
    {
        if (immediate)
        {
            ...
            Mod.Log.Msg("[State] Netplay SetFrame patch resumed after bivouac");
            return;
        }
    ...
    private void UpdateDeferredPatchResume()
    {
        ...
        _patchPaused = false;
        NetplaySetFramePatch.Resume();
        Mod.Log.Msg("[State] Netplay SetFrame patch resumed after bivouac (save-seal grace elapsed)");
    }
    ...
    public void LogPhase(string phase)
    {
        ...
        Mod.Log.Msg(
            $"[BivouacDebug] phase={phase} elapsed={elapsed:0.0}s scene='{Mod.Instance.CurrentScene ?? ""}' " +
            $"lastGameplay='{Mod.Instance.LastGameplayScene ?? ""}' local={Mod.Instance.LocalState} suspended={_suspended} " +
