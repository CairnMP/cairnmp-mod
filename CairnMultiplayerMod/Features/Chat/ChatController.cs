...
internal sealed class ChatController
{
    ...
    private void Open()
    {
        ...
        _historyIndex = -1;
        _draft = "";
        InputApi.ReconcileGameplayInput(true);   // immediate (the per-frame reconcile follows)
    }
    ...
    private void Close()
    {
        _isOpen = false;
        _input = "";
        InputApi.ReconcileGameplayInput(false);  // immediate; per-frame reconcile = safety net
    }
