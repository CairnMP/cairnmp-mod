...
public static class RemotePlayerManager
{
    ...
    public static bool TryGetGhostHarnessAttachPosition(int playerId, out Vector3 pos)
    {
        ...
        try
        {
            ...
            return harness != null && RopeApi.TryGetHarnessAttachPosition(harness, out pos);
        }
        catch
        {
            return false;
    ...
    public static void UpdateAll(NetworkManager net, PlayerState localState)
    {
        foreach (var kv in net.RemotePlayers)
        {
            ...
            {
                NetplayAnimationApi.CallNetplaySetFrame(entry.NrpComponent, rp.Id, rp.Name, rp.PlayerFrame);

                if (rp.HasClimbotFrame)
                {
                    NetplayAnimationApi.CallNetplayClimbotSetFrame(entry.NrpComponent, rp.Id, rp.ClimbotFrame);
        ...
        CosmeticApi.TickGhostGloveRigs();
        ...
        foreach (var kv in net.RemotePlayers)
        {
            ...
            if (entry.IsRealModel && entry.NrpComponent != null)
                NetplayAnimationApi.SetGhostNameVisible(entry.NrpComponent, ShowNames);
        }
    }
    ...
    private static void ApplyPendingCosmetics(NetworkManager net)
    {
        foreach (var kv in net.RemotePlayers)
        {
            ...
            if (!entry.HasAppliedCosmeticState || entry.LastAppliedGloves != glovesOn)
            {
                ...
                bool ok = CosmeticApi.SetGhostGloveMesh(entry.NrpComponent, glovesOn);
                CosmeticApi.SetGhostGlowingGloves(entry.NrpComponent, glovesOn);
                if (ok)
                {
    ...
    private static void SpawnGhost(int id, string name, RemotePlayer rp)
    {
        ...
        try
        {
            ...
            var prefab = UseNativePlayerPrefab
                ? NetplayAnimationApi.TryGetNetplayClimberPrefab()
                : null;
