...
/// carried over from Episure, cf. RopeApi.UpdateRopeTeamAnchor).
/// </summary>
internal sealed class RopeCoupleController
{
    ...
    internal RopeCoupleController(NetworkManager network)
    {
        _network = network;
    }
    ...
    private void TickRopeSafety()
    {
        ...
        bool inGame = Mod.Instance.LocalState == PlayerState.InGame;
    ...
    private void TearDownAllLocalRopeLinks(string reason)
    {
        ...
        Mod.Log.Msg($"[RopeCouple] Auto-unclipped {partners.Count} link(s): {reason}");
    }
    ...
    private void HandleRopeClipInput()
    {
        ...
        if (!found)
        {
            ...
            Mod.Log.Msg($"[RopeCouple] E pressed — no eligible ghost within {RopeClipRangeMeters:F0}m " +
        ...
        if (RopeLinkState.IsLinked(self, bestId))
        {
            ...
            Mod.Log.Msg($"[RopeCouple] Unclip from player {bestId}");
            return;
        }
        ...
        Mod.Log.Msg($"[RopeCouple] Clip to player {bestId} at {bestDist:F1}m");
    }
    ...
    private void TickRopeTeam()
    {
        ...
        RopeApi.UpdateRopeTeamAnchor(partner, partnerAnchor);
    }
