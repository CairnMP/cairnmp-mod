...
internal sealed class SketchMultiplayerPanel : IMultiplayerPanel
{
    ...
    private void BuildHost(GameObject body)
    {
        ...
        var slotsCell = SketchUiKit.RowStrip(body.transform, "Slots", 66f);
        _slotsLabel = SketchUiKit.Stepper(slotsCell, _slots.ToString(),
            (UnityAction)(() => AdjustSlots(-1)), (UnityAction)(() => AdjustSlots(+1)));

        var visCell = SketchUiKit.RowStrip(body.transform, "Visibility", 4f);
    ...
    {
        var codeCell = SketchUiKit.RowStrip(body.transform, "Lobby code", 118f);
    ...
    private void RenderBrowseRows()
    {
        ...
        if (count > MaxBrowseRows)
            ModLog.Debug($"[Sketch] browse: {count} lobbies, showing {MaxBrowseRows} (no scroll in v1).");
    }
