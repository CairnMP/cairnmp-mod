...
internal sealed class ExtensionRuntime
{
    ...
    private readonly ConcurrentQueue<Func<bool>> _completionCallbacks = new();

    // Poll tasks from the game loop, rather than scheduling game code on the thread pool.
    internal void ObserveCompletion<T>(Task<T> task, Action<Task<T>> handler)
    {
        if (Environment.CurrentManagedThreadId == _mainThreadId && task.IsCompleted)
        {
            handler(task);
            return;
        }
        _completionCallbacks.Enqueue(() =>
        {
    ...
    private void DrainCompletionCallbacks()
    {
        if (Environment.CurrentManagedThreadId != _mainThreadId)
            throw new InvalidOperationException("Extension callbacks must be pumped on the game thread.");
        var count = _completionCallbacks.Count;
        for (int i = 0; i < count && _completionCallbacks.TryDequeue(out var callback); i++)
        {
            try { if (!callback()) _completionCallbacks.Enqueue(callback); }
            catch (Exception ex) { _bridge?.ReportExtensionFailure("cairnmp.api", $"Completion callback failed: {ex.Message}"); }
        }
    }
    ...
    internal void Detach()
    {
        ...
        if (Environment.CurrentManagedThreadId == _mainThreadId) DrainCompletionCallbacks();
    ...
    internal void Tick()
    {
        ExpirePendingCommands(DateTime.UtcNow);
    ...
    internal void ExpirePendingCommands(DateTime now)
    {
        ...
        {
            if (pair.Value.Deadline > now) continue;
    ...
    {
        if (handlers == null) return;
        foreach (var handler in handlers.GetInvocationList().OfType<Action<T>>())
        {
            try { handler(value); }
            catch (Exception ex) { _bridge?.ReportExtensionFailure("cairnmp.api", $"{eventName} subscriber failed: {ex.Message}"); }
        }
    ...
    {
        if (handlers == null) return;
        foreach (var handler in handlers.GetInvocationList().OfType<Action<T1, T2>>())
        {
