...
internal static unsafe partial class BivouacDiagnostics
{
    ...
    private static string BuildInputManagerDebug()
    {
        try
        {
            var manager = InputInterop.FindInputManager();
            if (manager == null)
    ...
    private static string DescribeTransform(Transform transform)
    {
        ...
        {
            return DescribeInspectionError(exception);
        }
    }
    ...
    private static string DescribeInspectionError(Exception exception)
        => $"error:{exception.GetType().Name}:{GameInterop.FirstLine(exception.Message)}";
    ...
    private static object GetCurrentModelSafe(TapingFingersManager taping)
    {
        ...
        {
            ModLog.SuppressedException("bivouac.read-current-taping-model", exception);
            return null;
        }
    }
}
