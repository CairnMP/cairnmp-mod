...
/// </summary>
internal static unsafe class TimeInterop
{
    ...
    private static int EnsureIsAsleepOffset(MonoBehaviour psf)
    {
        ...
        try
        {
            ...
            foreach (var candidate in IsAsleepFieldCandidates)
            {
                ...
                ModLog.Debug($"[TimeSync] Resolved PlayerStateFeedbacks IsAsleep field '{candidate}' @ offset {_isAsleepFieldOffset}");
                return _isAsleepFieldOffset;
            }

            _isAsleepFieldOffset = -2;
            ModLog.Warning("[TimeSync] PlayerStateFeedbacks IsAsleep field not found — sleep detection disabled.");
        }
        catch (Exception ex)
        {
            _isAsleepFieldOffset = -2;
            ModLog.Warning($"[TimeSync] IsAsleep field resolution failed: {ex.GetType().Name}: {ex.Message}");
        }
